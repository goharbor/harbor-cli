...
Note: For custom schedules, if you provide a 5-field cron expression, the CLI will automatically add a leading "0" for the seconds field to create the required 6-field format.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			log.Infof("Updating scan all schedule to type: %s", scheduleType)
...
func updateScheduleToNone() error {
	log.Info("Setting scan all schedule to None (disabled)")
	...
	log.Info("Successfully disabled scan all schedule")
	return nil
}

func updatePredefinedSchedule(scheduleType string) error {
	log.Infof("Setting scan all schedule to %s", scheduleType)
	...
	log.Infof("Successfully set scan all schedule to %s", scheduleType)
	return nil
}

func updateCustomSchedule(cron string) error {
	if cron == "" {
		log.Info("Opening interactive form for custom schedule configuration")
	...
	log.Infof("Setting scan all schedule with custom cron expression: %s", cron)
	...
	log.Info("Successfully set scan all schedule with custom cron expression")
	return nil
}
...
func validateCron(cron string) error {
	...
	if len(fields) < 6 {
		if len(fields) == 5 {
			log.Infof("Converting 5-field cron to 6-field by adding '0' for seconds")
