...
func EditGCSchedule(cron *string, deleteUntagged *bool, dryRun *bool, promptCron bool) {
	...
	groups = append(groups, huh.NewGroup(
		huh.NewConfirm().
			...
			Description("If true, any untagged artifacts will be deleted during GC.").
			Value(deleteUntagged),
		huh.NewConfirm().
			Title("Dry run?").
			Description("If true, GC will simulate the cleanup process without deleting actual blobs.").
			Value(dryRun),
	))
